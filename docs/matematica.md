...
| **Punteggio logaritmico e test per rotazione** | una mappa di probabilità si giudica con il log della densità che dava al punto vero (regola di punteggio propria: non si alza barando, premia insieme precisione e giustezza). Per provare che il luogo conta: la stessa mappa ruotata attorno a casa tiene identiche le distanze e toglie solo la direzione; p per permutazione, un angolo a caso per unità. L'unità è il gatto, non la posizione (quelle dello stesso gatto non sono indipendenti) | Gneiting e Raftery 2007 (regole di punteggio proprie) | in uso: L5 in `MISURE.md`, `proto/gps/score.py` |
